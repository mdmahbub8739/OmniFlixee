...
if ('navigation' in window) {
  ...
  function openSourceModal() {
    ...
    if (isAnime) {
      const order = [['sub', 'Sub'], ['dub', 'Dub'], ['hindi', 'Hindi']];
      const groups = [];
      const used = new Set();
      ...
      if (rest.length) groups.push({ label: 'Other', rows: rest });
      primaryGroupsHtml = groups.map(g => `
        <div class="src-group">
          <div class="src-group__label">${g.label} servers <span class="src-pill">${g.rows.length}</span></div>
          <div class="source-rows">${g.rows.map(rowHtml).join('')}</div>
        </div>`).join('');
    } else {
      primaryGroupsHtml = primary.length ? `
        <div class="src-group">
          <div class="src-group__label">Servers <span class="src-pill">${primary.length}</span></div>
          <div class="source-rows">${primary.map(rowHtml).join('')}</div>
        </div>` : '';
    }
    ...
    m.innerHTML = `
      <div class="source-modal__panel">
        ...
        ${matchRows ? `
          <div class="src-group">
            <div class="src-group__label"><i class="ri-links-line"></i> Add source <span class="src-pill">${animeWorkers.length}</span></div>
